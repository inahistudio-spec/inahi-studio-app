...
def enabled(c):
    if not has_table(c, "saas_migrations"):
        return False
...
def upgrade(connect):
    ...
    with closing(connect()) as c, c:
        if getattr(c, "dialect", "sqlite") != "sqlite" or has_table(c, "alembic_version"):
            raise ValueError("Base gestionada: use db-upgrade con --report-file")
        c.execute("BEGIN IMMEDIATE")
        return upgrade_connection(c)


def upgrade_connection(c):
    if enabled(c):
...
def downgrade(connect):
    ...
    with closing(connect()) as c, c:
        if getattr(c, "dialect", "sqlite") != "sqlite" or has_table(c, "alembic_version"):
            raise ValueError("Base gestionada: use db-downgrade con --report-file")
        c.execute("BEGIN IMMEDIATE")
        return downgrade_connection(c)


def downgrade_connection(c):
    if not enabled(c):
        return False
